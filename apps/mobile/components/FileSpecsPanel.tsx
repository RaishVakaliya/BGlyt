...
export const FileSpecsPanel = React.memo(({ sourceImage, result }: FileSpecsPanelProps) => {
      ...
      <View className="gap-2">
        <View className="flex-row justify-between items-center py-1 border-b border-emerald-100/20">
          ...
          <Text
            ...
            numberOfLines={1}
            accessibilityLabel={`File name: ${result ? `no_bg_${sourceImage.fileName || "image.png"}` : sourceImage.fileName}`}
          >
            {result
              ? `no_bg_${sourceImage.fileName || "image.png"}`
        ...
        <View className="flex-row justify-between items-center py-1 border-b border-emerald-100/20">
          <View className="flex-row items-center">
            <Feather name="maximize" size={13} color="#059669" className="mr-2" />
            <Text className="text-slate-500 text-xs font-semibold">Dimensions</Text>
          </View>
          <Text
        ...
        <View className="flex-row justify-between items-center py-1 border-b border-emerald-100/20">
          <View className="flex-row items-center">
            <Feather name="database" size={13} color="#059669" className="mr-2" />
          ...
          >
            {result ? formatFileSize(processedSize) : formatFileSize(sourceImage.fileSize)}
          </Text>
        </View>
        ...
        <View className="flex-row justify-between items-center py-1 border-b border-emerald-100/20">
          <View className="flex-row items-center">
            ...
            <Text className="text-slate-500 text-xs font-semibold">Format</Text>
          </View>
          <Text
          ...
          >
            {result ? "PNG (Transparent)" : (sourceImage.mimeType?.split("/")[1] || "JPEG")}
          </Text>
        </View>
        ...
        {result && result.durationMs && (
          <View className="flex-row justify-between items-center py-1">
            <View className="flex-row items-center">
              <Feather name="clock" size={13} color="#059669" className="mr-2" />
              <Text className="text-slate-500 text-xs font-semibold">Processing Time</Text>
            </View>
            <Text
              className="text-slate-800 text-xs font-bold"
